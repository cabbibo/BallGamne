...
float2 opS( float2 d1, float2 d2 )
{
    return -d1.x > d2.x ? float2( -d1.x , d1.y ) : d2 ;
      ...
      float2 map( in float3 pos ){
        ...
        float n = noise( pos * float3( 1. , 2. , 1. ) *5.0 * ( 1. + sin( _Time.x * 2.) * .4 ) + float3( _SinTime.x , _SinTime.y , _SinTime.z ) );
        ...
        float3 modPos = opRepPos( pos * float3( 1. , 2. , 1. ) , float3( .3 , .3 , .3 ) );
        res2 = float2( sdSphere( modPos , .1), 2.);
